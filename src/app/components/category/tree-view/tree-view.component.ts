...
//   nodeSelected = output<number | null>(); // اضافه کردن خروجی
...
export class TreeViewComponent {
  ...
  constructor() {
    // استفاده از effect برای به روز رسانی خودکار categories
    effect(() => {
  ...
  loadNode(event: any) {
    if (event.node && !event.node.children?.length) {
      this.queryClient.fetchQuery({
        queryKey: ['category-children', event.node.data.id],
        queryFn: () =>
          lastValueFrom(this.categoryService.getCategoryChildren(event.node.data.id))
            .then(children => this.mapCategoriesToTreeNodes(children)),
      }).then(children => {
        event.node.children = children;
        this.categories.update(cats => [...cats]);
        this.messageService.add({ 
          severity: 'success', 
          summary: 'موفق', 
          detail: 'زیرمجموعه‌ها با موفقیت لود شدند', 
          life: 3000 
        });
      }).catch(() => {
        this.messageService.add({ 
